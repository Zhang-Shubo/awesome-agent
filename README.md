...
- **`web-react/`** 是面板的 React 版(玻璃拟态启动台:App/Agent 图标 + 悬浮详情 + 编辑模式,新增 APP 只填链接由 claude 解析;长按空白进编辑模式后图标与小组件卡片可拖动排序,顺序存面板机跨设备生效;登记簿声明 `widget-api` 的 app 会出常驻小组件卡,约定见 docs/07):`cd web-react && npm install && npm run build` 后,server 自动优先托管构建产物;不构建则用 `web/static` 零依赖版,两版共用同一套 API。面板里有只会溜达的桌宠噜噜(lulu-capybara-2,来自 [petdex.dev](https://petdex.dev),MIT):点它会跳,想换宠物就替换 `web-react/src/pet-spritesheet.webp`(Codex sprite v2 同格式),删掉该文件即无宠物。
- **手机上当 app 用(PWA)**:面板自带 manifest 与主屏幕图标,iPhone Safari「分享 → 添加到主屏幕」/安卓 Chrome「安装应用」后全屏独立运行,观感与原生 app 一致,无需打包上架;登录会话时长由 Access 应用的 Session Duration 决定。壳文件在 `web-react/public/`(manifest.webmanifest + apple-touch-icon.png + icon-512.png)。
