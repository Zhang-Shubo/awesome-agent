...
- **`web-react/`** 是面板的 React 版(玻璃拟态启动台:APP/Agent 图标 + 悬浮详情 + 编辑模式,新增 APP 只填链接由 claude 解析):`cd web-react && npm install && npm run build` 后,server 自动优先托管构建产物;不构建则用 `web/static` 零依赖版,两版共用同一套 API。面板里有只会溜达的桌宠噜噜(lulu-capybara-2,来自 [petdex.dev](https://petdex.dev),MIT):点它会跳,想换宠物就替换 `web-react/public/pet/spritesheet.webp`(Codex sprite v2 同格式),删掉该文件即无宠物。
